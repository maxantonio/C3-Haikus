...
var StockTools = function (raiz, periodos) {
    ...
    function init() {
        ...
        inicio.html('<span>' + i18n.t("from") + ':</span><input type="text" name="inicio" id="inicio" value="' + fechaInicio + '">');
        inicio.select("input").on("keypress", self.e_input_enter);

        var fin = divFechas.append("div").attr('class', 'divFechas');
        fin.html('<span>' + i18n.t("to") + ':</span><input type="text" name="fin" id="fin" value="' + fechaFin + '">');
        ...
        //console.log("paso los on change")
        header.append("div")
    ...
    self.m_establecer_fecha_inicial = function () {
        ...
        if (p[0][0] != null) {
            ...
            //console.log("paso el cantidad")

            var fechaInicio = self.m_getFechaInicio(tipo, cantidad);
            //console.log("paso el fechainicio")
    ...
    self.e_click_periodo = function () {
        ...
        var posLastDate = datos.json.length - 1;
        var fechaFin = parseDate(datos.json[posLastDate].date);
        var fechaInicio = self.m_getFechaInicio(tipo, cantidad);
        ...
        if (m_intervalo_Correcto(fechaInicio, fechaFin)) {
            ...
            if (comparando) {
                ...
                var r1 = self.m_calcular_comparacion(datos.json[1].date, 1, formatDate(fechaInicio), formatDate(fechaFin));
                ...
                chart.load({json: datos_a_cargar});
            } else {
    ...
    self.m_getFechaInicio = function (tipo, cantidad) {
        ...
        if(!fechaFin){
            var hoy = new Date();
            var fechaFin = hoy.getFullYear()+"-"+(hoy.getMonth()+1)+"-"+hoy.getDay();
        }
        ...
        var lfechaFin = parseDate(fechaFin);
        //console.log("aqui no llega");
        var fechaInicio = new Date(lfechaFin.getTime());

        switch (tipo) {
            case "dia":
                fechaInicio.setDate(lfechaFin.getDate() - cantidad);
                break;
            case "mes":
                fechaInicio.setMonth(lfechaFin.getMonth() - cantidad);
                break;
            case "anno":
                fechaInicio.setYear(lfechaFin.getFullYear() - cantidad);
                break;
            case "hasta_la_fecha":
                var ano = new Date().getFullYear();

                //1er dia del ano
                var dia_uno = new Date(lfechaFin.getFullYear(), 0, 1);
                var pos = bisect(datos.json, dia_uno);
                fechaInicio = parseDate(datos.json[pos].date);
                break;
        }
        return fechaInicio;
    }
    ...
    self.e_update_click = function () {
        ...
        if (m_intervalo_Correcto(fechaInicio, fechaFin)) {
            ...
            if (comparando) {
                ...
                //esta es los datos de comparacion de la 1ra empresa, que es la que se compara con las demas
                var r1 = self.m_calcular_comparacion(datos.json[0].simbol, 1, formatDate(fechaInicio), formatDate(fechaFin));
...
function m_intervalo_Correcto(fechaInicio, fechaFin) {
    ...
    var inicioDatos = parseDate(datos.json[0].date);
    var finDatos = parseDate(datos.json[datos.json.length-1].date);
